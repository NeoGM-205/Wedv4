...
async function registerSW() {
  ...
  try {
    const registration = await navigator.serviceWorker.register('/sw.js?v=1.10.0', { updateViaCache: 'none' });
...
if ('serviceWorker' in navigator) {
  ...
  navigator.serviceWorker.addEventListener('controllerchange', () => {
    if (sessionStorage.getItem('sw-reloaded-v1.10.0')) return;
    sessionStorage.setItem('sw-reloaded-v1.10.0', '1');
    location.reload();
  });
...
window.getAppCacheInfo = async () => {
  ...
  return { version: '1.10.0', controlled: !!navigator.serviceWorker?.controller, waiting: !!registration?.waiting, installing: !!registration?.installing, cacheKeys };
};
