...
async function registerSW() {
  ...
  try {
    const registration = await navigator.serviceWorker.register('/sw.js?v=1.9.3', { updateViaCache: 'none' });
...
if ('serviceWorker' in navigator) {
  ...
  navigator.serviceWorker.addEventListener('controllerchange', () => {
    if (sessionStorage.getItem('sw-reloaded-v1.9.3')) return;
    sessionStorage.setItem('sw-reloaded-v1.9.3', '1');
    location.reload();
  });
...
window.getAppCacheInfo = async () => {
  ...
  return { version: '1.9.3', controlled: !!navigator.serviceWorker?.controller, waiting: !!registration?.waiting, installing: !!registration?.installing, cacheKeys };
};
